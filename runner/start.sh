...
export REPO_ROOT HF_HOME INSPECT_LOG_DIR PLAYWRIGHT_BROWSERS_PATH PID_FILE
export VENV_PYTHON PATH INSPECT_BIN INSPECT_TOOL_SUPPORT_BIN
...
with sync_playwright() as p:
    chromium = Path(p.chromium.executable_path)
if not chromium.is_file():
    raise RuntimeError(f"Playwright Chromium is not installed: {chromium}")
PY
}
